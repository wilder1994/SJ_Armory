    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <form method="GET" class="mb-4 flex items-center gap-2">
                ...
                <select name="custodian_user_id" class="rounded-md border-gray-300 text-sm">
                    <option value="">{{ __('Todos') }}</option>
                    @foreach ($responsibles as $responsible)
                        <option value="{{ $responsible->id }}" @selected($custodianId === $responsible->id)>
                            {{ $responsible->name }}
                        </option>
                    @endforeach
                </select>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($weapons as $weapon)
                                <tr>
                                    ...
                                    <td class="px-3 py-2">{{ $weapon->activeCustody?->custodian?->name ?? '-' }}</td>
                                    <td class="px-3 py-2">{{ $weapon->activeCustody?->start_at?->format('Y-m-d H:i') }}</td>
                                </tr>
                            @empty
                                <tr>
