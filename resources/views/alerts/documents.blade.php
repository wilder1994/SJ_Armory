<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-semibold mb-3">{{ __('Revalidaciones próximas') }}</h3>
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2 text-left font-medium text-gray-600">{{ __('Arma') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($revalidations as $doc)
                                <tr>
                                    <td class="px-3 py-2">{{ $doc->weapon?->internal_code }}</td>
                                    <td class="px-3 py-2">{{ $doc->doc_type }}</td>
                                    <td class="px-3 py-2">{{ $doc->revalidation_due_at?->format('Y-m-d') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-3 py-6 text-center text-gray-500">
                                        {{ __('Sin revalidaciones próximas.') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
